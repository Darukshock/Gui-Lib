...
execute store result score B ui.val run data get entity @s Items
data modify entity @s Items insert 0 from storage ui temp[]
execute store result score C ui.val run data get entity @s Items
execute if score B ui.val = C ui.val run return run function ui:menu/on_right_or_partial_drop
...
# ! IN = {storage ui in[0].components.minecraft:custom_data."ui.item".right}
